...
export default function DocumentPreview({ data }: Props) {
  ...
  const clientName =
    `${data.clientFirst} ${data.clientLast}`.trim() || "FIRST LAST";
  ...
  if (
    ...
    (data.bankName || data.bankAcc)
  ) {
    payBlocks.push(
      <div key="cheque" style={{ marginBottom: 10 }}>
        <div style={S.payTitle}>Physical Cheque</div>
        <div style={S.detail}>
          Bank: {data.bankName || "BANK NAME"}
          <br />
          Account: {data.bankAcc || "0000000000"}
          <br />
          {data.chequeNo && (
            <>
  ...
  if (data.activePayments.includes("paypal")) {
    payBlocks.push(
      <div key="paypal" style={{ marginBottom: 10 }}>
        ...
        <div style={S.detail}>
          Username: {data.paypalUser || "@username"}
          <br />
          Transaction ID: {data.paypalTx || "TXID-000"}
          <br />
  ...
  if (data.activePayments.includes("bank")) {
    payBlocks.push(
      <div key="bank" style={{ marginBottom: 10 }}>
        ...
        <div style={S.detail}>
          Bank: {data.bkName || "Bank Name"}
          <br />
          PAYBILL: {data.bkAcc || "0000000000"}
          <br />
          {data.bkSwift && (
            <>
              ACCOUNT NO.: {data.bkSwift}
              <br />
            </>
          )}
  ...
  if (data.activePayments.includes("mpesa")) {
    payBlocks.push(
      <div key="mpesa" style={{ marginBottom: 10 }}>
        ...
        <div style={S.detail}>
          {isReceipt ? "Paid To" : "Pay To"}:{" "}
          <strong>{data.mpNum || "0703119107"}</strong>
          <br />
          {isReceipt && (
            <>
              Transaction Code:{" "}
          ...
          {data.mpAmt && (
            <>
              Amount: <strong>{data.mpAmt}</strong>
            </>
          )}
        </div>
      </div>,
  ...
  return (
    ...
    >
      ...
      >
        <div
          style={{ padding: "32px 30px 24px", borderRight: "1px solid #ddd" }}
        >
          <div
            style={{
          ...
          >
            <div>
              <div style={S.metaLabel}>
                {isReceipt ? "Receipt To" : "Invoice To"}
              </div>
              <div style={S.metaValue}>
                {data.clientEntity || "ENTITY NAME"}
              </div>
            </div>
            <div>
              <div style={S.metaLabel}>
        ...
        <div style={{ borderRight: "1px solid #ddd" }}>
          <div style={S.leftSection}>
            ...
            <div style={S.detail}>
              ...
              {data.clientAddress || "Address, City, Country"}
              <br />
          ...
          <div style={S.leftSection}>
            ...
            <div style={S.detail}>
              ...
              {data.provAddress || "Address, City, Country"}
              <br />
          ...
          <div style={S.leftSection}>
            ...
            {payBlocks.length > 0 ? (
              ...
              <div style={S.detail}>No payment details added</div>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ padding: "20px 26px", borderBottom: "1px solid #ddd" }}>
            ...
            <div style={{ fontSize: 11, color: "#555", lineHeight: 1.7 }}>
              {data.description ||
                "A brief overview of the project, including its objectives and scope."}
            </div>
          </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "#111" }}>
                ...
                <th
                  style={{ ...S.th, textAlign: "center" as const, width: 60 }}
                >
                  Qty.
                </th>
          ...
          >
            <div
              style={{
                ...
                borderRadius: "50%",
                background:
                  "radial-gradient(circle at 40% 35%, #e8c96a, #b8932a 60%, #8a6a18)",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                boxShadow: "0 3px 12px rgba(184,147,42,0.4)",
      ...
      >
        ...
        <div style={{ padding: "16px 22px" }}>
          ...
          >
            The total {isReceipt ? "invoice" : "billed"} amount has been{" "}
            {isReceipt ? "fully paid by" : "invoiced to"}{" "}
            <strong style={{ color: "#777" }}>{clientName}</strong>
            {data.clientEntity ? (
              <>
                {" "}
                of{" "}
                <strong style={{ color: "#777" }}>{data.clientEntity}</strong>
              </>
            ) : (
              ""
            )}{" "}
            without any deductions. The bill total has been rounded off to the
