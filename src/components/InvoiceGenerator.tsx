...
export default function InvoiceGenerator() {
  ...
  const setItem = (id: number, key: keyof LineItem, value: string | number) => {
    setData(prev => ({
      ...prev,
      lineItems: prev.lineItems.map(i => i.id === id ? { ...i, [key]: value } : i),
    }))
  }

  const addItem = () => {
    const id = ++_itemCounter
    setData(prev => ({ ...prev, lineItems: [...prev.lineItems, { id, name: '', qty: 1, amount: '', extra: '' }] }))
  }
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      if (imgH <= A4_H + 2) {
        pdf.addImage(canvas.toDataURL('image/jpeg', 0.97), 'JPEG', 0, 0, imgW, imgH)
      } else {
        ...
        while (srcY < canvas.height) {
          ...
          pdf.addImage(slice.toDataURL('image/jpeg', 0.97), 'JPEG', 0, 0, imgW, (srcH / canvas.width) * imgW)
          srcY += pxPerPage
          page++
        }
      }
  ...
  const F = (label: string, key: keyof FormData, placeholder = '', type = 'text') => (
    <div style={S.field}>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '420px 1fr', minHeight: '100vh' }}>
      ...
      {/* ── FORM PANEL ── */}
      <aside style={{ background: 'var(--paper)', height: '100vh', overflowY: 'auto', position: 'sticky', top: 0, borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column' }}>

        {/* Header */}
        <div style={{ padding: '28px 28px 20px', borderBottom: '1px solid var(--border)', background: 'var(--cream)' }}>
          <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 900 }}>Doc Generator</div>
          <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 3, textTransform: 'uppercase', letterSpacing: 0.5 }}>Invoice & Receipt Builder</div>
          <div style={{ display: 'flex', marginTop: 18, background: 'var(--border)', borderRadius: 8, padding: 3, gap: 3 }}>
            {(['INVOICE', 'RECEIPT'] as const).map(t => (
              <button key={t} onClick={() => set('docType', t)} style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 20 }}>
          ...
          <Section label="Payment Method">
            <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' as const }}>
              {payMethods.map(m => (
                <button key={m.id} onClick={() => togglePayment(m.id)} style={{
                  padding: '6px 12px', border: '1px solid var(--border)', borderRadius: 20,
                  fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit',
                  background: data.activePayments.includes(m.id) ? 'var(--ink)' : 'var(--cream)',
                  color: data.activePayments.includes(m.id) ? '#fff' : 'var(--muted)',
            ...
            {data.activePayments.includes('cheque') && (
              <>
                <div style={S.row}>{F('Bank Name', 'bankName', 'BANK NAME')}{F('Account #', 'bankAcc', '0123456789')}</div>
                <div style={S.row}>{F('Cheque No.', 'chequeNo', '012233344444')}{F('Amount', 'chequeAmt', 'KES 00,000')}</div>
              </>
            )}
            {data.activePayments.includes('paypal') && (
              <>
                {F('PayPal Username', 'paypalUser', '@username')}
                <div style={S.row}>{F('Transaction ID', 'paypalTx', 'TXID-000000')}{F('Amount', 'paypalAmt', 'KES 00,000')}</div>
              </>
            )}
            {data.activePayments.includes('bank') && (
              <>
                {F('Bank Name', 'bkName', 'Bank Name')}
                <div style={S.row}>{F('Account #', 'bkAcc', '0000000000')}{F('SWIFT / IFSC', 'bkSwift', 'CODE')}</div>
          ...
          <Section label="Line Items">
            {data.lineItems.map(item => (
              <div key={item.id} style={{ background: 'var(--cream)', border: '1px solid var(--border)', borderRadius: 8, padding: 12, marginBottom: 10 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 65px 85px 28px', gap: 8, alignItems: 'start' }}>
                  <div style={S.field}>
                    <label style={S.label}>Description</label>
                    <input value={item.name} placeholder="Item name" onChange={e => setItem(item.id, 'name', e.target.value)} style={S.input} />
                  </div>
                  <div style={S.field}>
                    <label style={S.label}>Qty</label>
                    <input type="number" value={item.qty} min={1} onChange={e => setItem(item.id, 'qty', parseInt(e.target.value) || 1)} style={S.input} />
                  </div>
                  <div style={S.field}>
                    <label style={S.label}>Amount</label>
                    <input value={item.amount} placeholder="0,000" onChange={e => setItem(item.id, 'amount', e.target.value)} style={S.input} />
                  </div>
                  <button onClick={() => removeItem(item.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)', fontSize: 18, lineHeight: 1, padding: '6px 4px', alignSelf: 'center' }}>×</button>
                </div>
                <div style={{ marginTop: 8, paddingTop: 8, borderTop: '1px dashed var(--border)' }}>
                  <div style={S.field}>
                    <label style={S.label}>Extra Details (one per line)</label>
                    <textarea value={item.extra} placeholder="extra detail&#10;another detail" rows={2}
                      onChange={e => setItem(item.id, 'extra', e.target.value)}
            ...
            <button onClick={addItem} style={{
              ...
              fontFamily: 'inherit', fontSize: 13, color: 'var(--muted)', cursor: 'pointer', width: '100%',
            }}>
          ...
          <Section label="Totals">
            <div style={S.row}>
              <div style={S.field}>
                ...
                <input value={data.currency} placeholder="KES" onChange={e => set('currency', e.target.value)} style={{ ...S.input, maxWidth: 80 }} />
              </div>
              <div style={S.field}>
                <label style={S.label}>VAT Rate (%)</label>
                <input type="number" value={data.taxRate} min={0} max={100} step={0.5} onChange={e => set('taxRate', parseFloat(e.target.value) || 0)} style={S.input} />
              </div>
            </div>
      ...
      {/* ── PREVIEW PANEL ── */}
      <main style={{ background: '#2a2720', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px 40px 80px', overflowY: 'auto' }}>
        <div style={{ fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', color: '#6b6560', marginBottom: 24, alignSelf: 'flex-start' }}>
          Live Preview
        </div>
...
function Section({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ padding: '20px 28px 16px', borderBottom: '1px solid var(--border)' }}>
      <div style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: 1.5, color: 'var(--gold)', marginBottom: 14 }}>
        {label}
      </div>
      {children}
    </div>
...
const S = {
  ...
  label: { fontSize: 11, fontWeight: 500, color: 'var(--muted)', letterSpacing: 0.3 },
  input: {
